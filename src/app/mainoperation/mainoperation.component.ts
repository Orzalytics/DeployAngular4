...
export function onCalculateData(){
	...
	var maxArray = [];
	var firstDate = new Date(Globals.g_GlobalStatic.startDate);
...
export function  getTransactionData(response){
	...
	for (var i = 0; i < TransactionList.length; i ++){
		var transDate = new Date(TransactionList[i].date_value_transaction);
		...
		tempArr.tDate = Globals.convertDate(TransactionList[i].date_value_transaction);
...
export function CalculatePortfolioData(){
	...
	for (var n = 0; n < Globals.g_DatabaseInfo.PortfolioList.length; n ++){
		...
		for (var i = 0; i < response.length; i ++){
			...
			var transactionDate = new Date(response[i].tDate);
			var dateIndex = -1;
		...
		for (var i = 0; i < response.length; i ++){
			for (var j = 0; j < Globals.g_DatabaseInfo.ListofPriceFund[0].ulen; j ++){
				var orgDate = new Date(response[i].tDate);
